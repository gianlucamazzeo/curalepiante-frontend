<script lang="ts">
  ...
  let searchQuery = '';
  let indoorFilter = false;
  let floweringFilter = false;
  let wateringFilter = '';
  
  // SEO data
  const seoData = {
    title: 'Catalogo Piante - Trova la pianta perfetta per te',
    description: 'Esplora il nostro catalogo di piante. Filtra per tipo, esigenze di luce, acqua e molto altro.',
    keywords: 'piante, catalogo piante, vendita piante online',
  ...
  function applyFilters() {
    ...
    if (indoorFilter) url.searchParams.set('indoor', 'true');
    else url.searchParams.delete('indoor');
  ...
  onMount(() => {
    ...
    indoorFilter = url.searchParams.get('indoor') === 'true';
    ...
    pianteStore.fetchPiante(null, initialFilters);
  });
</script>
...
<div class="bg-green-50 py-12">
  <div class="container mx-auto px-4">
    <h1 class="text-4xl font-serif font-bold text-emerald-800 mb-8 text-center">Catalogo Piante</h1>
    
    <!-- Filtri di ricerca -->
    <div class="bg-white p-6 rounded-lg shadow-md mb-8">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          <label for="search-input" class="block text-sm font-medium text-gray-700 mb-1">Ricerca</label>
          <input 
            id="search-input"
            type="text" 
        ...
        <div class="flex items-end">
          <button 
            ...
            class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500"
          >
            Applica filtri
          </button>
        </div>
    ...
    {:else if $pianteStore.piante.length === 0}
      <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 p-6 rounded-md my-8 text-center">
        ...
        <p>Non ci sono piante che corrispondono ai filtri selezionati. Prova a modificare i criteri di ricerca.</p>
      </div>
    {:else}
      ...
      <div class="mb-6 text-gray-600">
        <p>
          ...
          di {$pianteStore.pagination.total} piante
        </p>
      </div>
